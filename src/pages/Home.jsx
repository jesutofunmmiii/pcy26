...
// Maps the reference's `page` ids to router paths (scroll-to-top is handled
// globally in App by <ScrollToTop />).
const PAGE_PATHS = {
  home: '/',
  about: '/about',
  program: '/program',
    ...
    <div>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', background: 'var(--green-900)', minHeight: '100vh', display: 'flex', alignItems: 'center' }}>
        {heroSlides.map((src, i) => (
          <img key={src} data-no-reveal src={'/gallery/' + src} alt="" loading={i === 0 ? 'eager' : 'lazy'} decoding={i === 0 ? 'sync' : 'async'} fetchpriority={i === 0 ? 'high' : 'low'} style={{
        ...
        <div style={{ position: 'relative', padding: 'var(--space-8) clamp(24px, 6vw, 120px)', width: '100%' }}>
          <div style={{ maxWidth: 700 }}>
            ...
            <div className="rise rise-4" style={{ display: 'flex', gap: 16, marginTop: 'var(--space-6)', flexWrap: 'wrap', alignItems: 'center' }}>
              <Button variant="accent" size="sm" icon="download" iconPosition="end" onClick={downloadCompendium} className="glow-cta" style={{ borderRadius: 'var(--radius-pill)' }}>Download the compendium</Button>
      ...
      <section className="section" style={{ padding: 'var(--space-6) var(--layout-margin) var(--space-9)' }}>
        <div className="qa-cards" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--space-6)', paddingBottom: 'var(--space-7)', marginBottom: 'var(--space-8)', borderBottom: '1px solid var(--border-default)' }}>
          {facts.map((f) => (
        ...
        <div className="qa-stack" style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 'var(--space-8)', alignItems: 'flex-start' }}>
          ...
          <Card featureCorner corner="top-right" padding="var(--space-6)" style={{ position: 'sticky', top: 92, background: 'linear-gradient(160deg, var(--green-900) 0%, #0c3016 100%)', border: '1px solid rgba(240,193,75,0.18)', boxShadow: '0 24px 60px -24px rgba(4,34,12,0.55)' }}>
            ...
            <Button variant="accent" size="lg" icon="download" iconPosition="end" onClick={downloadCompendium} style={{ width: '100%', marginTop: 'var(--space-5)', justifyContent: 'center' }}>Download the compendium</Button>
...
function HomeCta({ onNavigate }) {
  return (
    <section style={{ position: 'relative', overflow: 'hidden', background: 'var(--green-900)' }}>
      <img data-no-reveal src="/assets/rising-arc-line.svg" alt="" style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', height: '70%', objectFit: 'cover', opacity: 0.4 }} />
...
function Press() {
  ...
  return (
    <section style={{ position: 'relative', overflow: 'hidden', background: 'var(--green-900)', padding: 'var(--space-9) 0' }}>
      <img data-no-reveal src="/assets/press-bg.webp" alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 40%' }} />
      ...
      <div style={{ position: 'relative' }}>
        <div className="section">
          ...
          <div className="qa-cards" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 'var(--space-5)', padding: 'var(--space-6) 0', borderTop: '1px solid rgba(255,255,255,0.18)', borderBottom: '1px solid rgba(255,255,255,0.18)', margin: 'var(--space-7) 0 var(--space-7)' }}>
            {numbers.map(([n, label]) => (
              <div key={label}>
...
function GalleryPreview({ onNavigate }) {
  const photos = GALLERY_PHOTOS.slice(0, 15);
    ...
    <section style={{ background: 'var(--green-900)', marginTop: 'var(--space-9)' }}>
      <div className="section" style={{ padding: 'var(--space-9) var(--layout-margin)' }}>
        ...
        <div className="pcy-masonry">
          {photos.map((src, i) => (
            <figure key={src}>
              <img src={'/gallery/' + src} alt={'Policy Conference for Youth 2026 — moment ' + (i + 1)} loading="lazy" decoding="async" />
            </figure>
          ))}
        </div>
      </div>
